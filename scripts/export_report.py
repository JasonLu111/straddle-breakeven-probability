...
REPORT_TITLE = "Phase 1 研究報告：市場事件研究（Market Event Research）"
...
def build_docx() -> Path:
    ...
    h("7. 下一步")
    body(
        "Phase 2 需要真實歷史選擇權資料（bid/ask, IV, OI, delta），以建立真正的 breakeven "
        "門檻、計算扣除交易成本後的真實 PnL，並重新檢驗 H2。"
    )
...
def build_pdf() -> Path:
    ...
    story.append(Paragraph("7. 下一步", h1_style))
    story.append(Paragraph(
        "Phase 2 需要真實歷史選擇權資料（bid/ask, IV, OI, delta），以建立真正的 "
        "breakeven 門檻、計算扣除交易成本後的真實 PnL，並重新檢驗 H2。", body_style))
